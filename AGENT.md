...
- 支持通过指令查询任意比赛（正在进行、已完赛、未开赛）的深度数据：
  - `/hltv match <ID或战队名/别名>`：查询全场总比分、选图情况及双方全部选手的综合数据。
  - `/hltv match <ID或战队名/别名> <图号>`：查询单图（如图 1、图 2）的比分、半场比分及专属单图全员 KDA/Rating 数据。
  - `/hltv match <ID或战队名> -r`：附加 `-r` 或 `刷新` 标记，强制先清除服务端缓存后再执行查询。

### 3.5 常用战队中文别名与缩写匹配 (`resolve_team_aliases`)
- 国内 CS 社区普遍习惯使用中文战队外号与英文缩写，插件内置完备的战队别名映射库（包含 40+ 支知名战队）：
  - `Spirit` -> 绿龙、ts
  - `Vitality` -> 小蜜蜂、小黄蜂
  - `FaZe` -> 银河战舰、大表哥
  - `NAVI` -> 邪剑仙、navi
  ...
  - `Virtus.pro` -> VP、vp、大狗
  - `Heroic` -> 英雄、heroic
  - 以及 LVG、EF (土耳其神车)、BESTIA (野兽) 等全套别名。
- **匹配防误触策略**：对于长度 $\le 3$ 的短英文缩写（如 `ts`, `vp`, `ra`, `a队`），必须严格全词匹配，避免作为子串误伤其他战队；较长中文别名支持包含与双向模糊匹配。

### 3.6 陈旧半场缓存自愈与服务端缓存刷新 (`/hltv refresh`)
- **痛点**：上游 API 抓取端在比赛进行中会产生中间态缓存（如半场比分）。若比赛完赛后详情缓存未及时过期，后续查询可能会错误返回半场数据。
- **自动赛果交叉对齐**：`get_match_detail` 在读取到状态为 `live` 或比分不完整的比赛时，自动联动近 7 日的完赛赛果库（`get_results`）。若赛果库已确认该场比赛完赛，插件自动纠偏 `status = "finished"`，同步最终官方大比分，并在返回提示中明确注明已自动同步最新赛果。
- **服务端主动缓存清除**：
  - 增加指令 `/hltv refresh`（别名 `/hltv 刷新`、`/hltv 清除缓存`），主动调用上游接口 `DELETE /api/v1/cache` 清除陈旧缓存，令服务端重新拉取即时数据。

### 3.7 单场详情限流回退与比赛 ID 稳定检索
- **痛点**：HLTV 官方存在 Cloudflare 防护机制。当上游无头浏览器处于冷却期（900 秒）时，单场详情接口 `/api/v1/matches/{id}` 会返回 503 状态码（ChallengeBlocked），过去会导致纯数字 ID 查询直接报“未找到对应比赛”。
- **三层回退检索保障**：
  - 无论何时，若单场详情返回 503 或 None，插件自动回退扫描近 7 日赛果库（`get_results`）、当前进行中列表（`get_live_matches`）与近 7 日赛程库（`get_matches`）。
  - 一旦匹配到对应的比赛 ID，即刻构造对局数据并提示用户“已从赛程/赛果库呈现对局数据，大比分已确认”，彻底解决纯数字 ID 查询稳定落空的问题。
